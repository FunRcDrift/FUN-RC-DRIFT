  ...
  el.innerHTML = `
    <nav class="nav">
      <a class="brand" href="index.html">
        <img src="assets/logo-frd.jpeg" alt="">
        <span>FRD Fun RC Drift</span>
      </a>
      <button class="mobile-menu" aria-label="Ouvrir le menu">☰</button>
      <div class="nav-links">
        <a href="pilotes.html">Pilotes</a>
        <a href="admin.html" data-auth>Admin</a>
        <a href="espace-pilote.html" data-auth>Mon profil</a>
        <a href="espace-pilote.html" data-guest>Connexion</a>
        <button data-logout hidden>Déconnexion</button>
      </div>
    </nav>
  `;
...
export function setupNotice(parent) {
  if (IS_CONFIGURED) return;

  const notice = document.createElement('div');
  notice.className = 'setup-notice';
  notice.innerHTML = '<strong>Mode aperçu</strong> — Connecte ce site à Supabase dans <code>js/config.js</code> pour activer les comptes, les profils et les photos.';
...
export async function initShell() {
  ...
  document.querySelectorAll('[data-auth],[data-logout]').forEach(element => {
    element.hidden = !user;
  });
